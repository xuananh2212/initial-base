...
export class ValidationPipe implements PipeTransform<any> {
  ...
  async transform(value, metadata: ArgumentMetadata) {
    ...
    if (typeof value === 'object') {
      ...
      if (errors.length > 0) {
        const messages = await this.getMessage(errors, value?.lang);
        return {
          request: object,
      ...
      return {
        request: value,
        responseError: new ApiError(
          ResponseCodeEnum.UNSUPPORT_MEDIA_TYPE,
          await this.i18n.translate('error.UNSUPPORT_MEDIA_TYPE'),
        ).toResponse(),
      };
    }
  }
  ...
  private async getMessage(
    ...
    for (const error of errors) {
      if (!error.children || !error.children.length) {
        ...
        const errorMessage = patternFinded
        ...
        errorMessages.push({
          field: replacements.property,
          message: isArray(errorMessage) ? errorMessage[0] : errorMessage,
        });
      } else {
        const childErrorMessages = await this.getMessage(error.children, lang);
